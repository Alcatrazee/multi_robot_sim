...
*/
uint16_t A_star_path_finding(int8_t map[Height][Width], uint16_t start_point[2],
                             uint16_t end_point[2], uint16_t Path[length][fn]);
inline int Get_heuristic_function(uint16_t current_point[2], uint16_t Goal[2]);
int check_end_point_in_OpenSet(uint16_t end_point[2],
...
void Expend_current_point(int8_t map[Height][Height], uint16_t current_point[3],
                          uint16_t OpenSet[OpenSet_array_size][7],
                          uint16_t ClosedSet[closedset_array_size][7],
                          uint16_t end_point[2], uint16_t start_point[2],
                          uint16_t closed_list_counter);
bool Is_OpenSet_empty(uint16_t OpenSet[OpenSet_array_size][7]);
void Init(uint16_t OpenSet[OpenSet_array_size][7]);
bool Is_in_closed_set(uint16_t point[2],
                      uint16_t ClosedSet[closedset_array_size][7],
                      uint16_t closed_list_counter);
short Is_in_Open_set(uint16_t point[2],
                     uint16_t Openset[OpenSet_array_size][7]);
uint16_t Find_parent_index(uint16_t parent[2],
                           uint16_t ClosedSet[closedset_array_size][7],
                           uint16_t CloseSet_index);
uint16_t Get_path(uint16_t ClosedSet[closedset_array_size][7],
                  uint16_t CloseSet_index, uint16_t Path[length][fn],
                  uint16_t start_point[2]);
uint16_t Check_start_end_point(int8_t map[Height][Width],
                               uint16_t start_point[2], uint16_t end_point[2]);
void Transform_coordinate(float start[2], float end[2], uint16_t start_point[2],
                          uint16_t end_point[2]);
bool map_rec_callback(multiple_rb_ctrl::dynamic_path_srv::Request &req,
                      multiple_rb_ctrl::dynamic_path_srv::Response &res);
bool Is_in_open_or_closed_set(uint16_t point[2],
                              uint16_t OpenSet[OpenSet_array_size][7],
                              uint16_t ClosedSet[closedset_array_size][7],
                              uint16_t closed_list_counter);
void find_avaiable_point(uint16_t point[2]);
void generate_map(void);
void generate_occupied_map(const int8_t using_map[Height][Width],
                           int8_t map_to_copy[Height][Width],
                           int8_t robot_coor[4][2], uint8_t current_id);
...
void generate_occupied_map(const int8_t using_map[Height][Width],
  ...
  for (int used_robot_num = 0; used_robot_num < 4; used_robot_num++) {
    ROS_INFO("%d %d", robot_coor[used_robot_num][0],
             robot_coor[used_robot_num][1]);
    if (used_robot_num == current_id - 1)
      continue;
    // only those are close enough will be treated as obstacles.
    if (abs(robot_coor[used_robot_num][0] - robot_coor[current_id - 1][0]) <=
            obs_distance &&
        abs(robot_coor[used_robot_num][1] - robot_coor[current_id - 1][1]) <=
            obs_distance)
      map_to_copy[robot_coor[used_robot_num][0]]
                 [robot_coor[used_robot_num][1]] = obstacle_mark;
    map_to_copy[spawn_pos[used_robot_num][0]][spawn_pos[used_robot_num][1]] =
        obstacle_mark;
  }
  // show map
  /*   if (current_id == 2)
...
void generate_map(void) {
  for (int _row = 0; _row < 19; _row++) {
    for (int _col = 0; _col < 19; _col++) {
      if (((_row >= 2 && _row <= 8) || (_row >= 10 && _row <= 16)) &&
          (_col == 2 || _col == 5 || _col == 8 || _col == 11 || _col == 14)) {
...
uint16_t A_star_path_finding(int8_t map[Height][Width], uint16_t start_point[2],
  ...
  while (check_end_point_in_OpenSet(end_point, OpenSet) == -1) {
    if (!Is_OpenSet_empty(OpenSet)) {
      ...
      CloseSet_index++;
      Expend_current_point(map, current_point, OpenSet, ClosedSet, end_point,
                           start_point, CloseSet_index);
    } else {
      return 0;
    }
  }
...
uint16_t Get_path(uint16_t ClosedSet[closedset_array_size][7],
  ...
  while (!(Path[Path_index - 1][row] == start_point[row] &&
           Path[Path_index - 1][column] == start_point[column])) {
...
uint16_t Find_parent_index(uint16_t parent[2],
  ...
  for (int i = CloseSet_index - 1; i >= 0; i--) {
    if (parent[row] == ClosedSet[i][row] &&
        parent[column] == ClosedSet[i][column]) {
      return i;
    }
  }
  return 0;
...
short Is_in_Open_set(uint16_t point[2],
  ...
  for (int i = 0; i < max_openset_index; i++) {
    if (Openset[i][row] == point[0] && Openset[i][column] == point[1] &&
        Openset[i][valid_bit] == valid) {
      return i;
    }
  }
  return -1;
...
bool Is_in_open_or_closed_set(uint16_t point[2],
  ...
  for (int i = 0; i < max_openset_index; i++) {
    if (OpenSet[i][valid_bit] == 1 &&
        (OpenSet[i][row] == point[0] && OpenSet[i][column] == point[1])) {
      return true;
    }
  }
  for (int i = 0; i < closed_list_counter; i++) {
...
void Expend_current_point(int8_t map[Height][Height], uint16_t current_point[3],
  ...
  if (current_point[0] > 0 &&
      !Is_in_closed_set(point, ClosedSet, closed_list_counter) &&
      map[point[0]][point[1]] != unknown_mark &&
      map[point[0]][point[1]] != obstacle_mark) {
    ...
    if (index != -1) // point is existed
    {
      if (Get_gn(current_point[2], normal) < OpenSet[index][gn]) {
        ...
        OpenSet[index][fn] =
            Get_heuristic_function(point, end_point) + OpenSet[index][gn];
  ...
  if (current_point[1] > 0 && current_point[0] > 0 && !Is_in_closed_set(point,
  ClosedSet, closed_list_counter) && map[point[0]][point[1]] != obstacle_mark &&
  map[point[0]][point[1]] != unknown_mark)
  {
    // create a point variable
    index = Is_in_Open_set(point, OpenSet);
    if (index != -1) // point is existed
    {
      if (Get_gn(current_point[2], hypotenous) < OpenSet[index][gn])
      {
        ...
        OpenSet[index][fn] = Get_heuristic_function(point, end_point) +
  OpenSet[index][gn]; OpenSet[index][parent_row] = current_point[row];
        OpenSet[index][parent_col] = current_point[column];
        OpenSet[index][valid_bit] = valid;
      }
    }
  ...
  if (current_point[1] < Width - 1 && current_point[0] > 0 &&
  !Is_in_closed_set(point, ClosedSet, closed_list_counter) &&
  map[point[0]][point[1]] != obstacle_mark && map[point[0]][point[1]] !=
  unknown_mark)
  {
    // create a point variable
    index = Is_in_Open_set(point, OpenSet);
  ...
  /* point[0] = current_point[0] + 1;
  point[1] = current_point[1] - 1;
  if (current_point[1] > 0 && current_point[0] < Height - 1 &&
  !Is_in_closed_set(point, ClosedSet, closed_list_counter) &&
  map[point[0]][point[1]] != obstacle_mark && map[point[0]][point[1]] !=
  unknown_mark)
  {
  ...
  if (current_point[1] < Width - 1 && current_point[0] < Height - 1 &&
  !Is_in_closed_set(point, ClosedSet, closed_list_counter) &&
  map[point[0]][point[1]] != obstacle_mark && map[point[0]][point[1]] !=
  unknown_mark)
  {
...
inline int Get_heuristic_function(uint16_t current_point[2], uint16_t Goal[2]) {
  // return sqrt((current_point[0] - Goal[0]) * (current_point[0] - Goal[0]) +
  // (current_point[1] - Goal[1]) * (current_point[1] - Goal[1])) * cost *
  // heuristic_scaler;
  return (abs(Goal[1] - current_point[1]) + abs(Goal[0] - current_point[0])) *
         cost * heuristic_scaler;
}
...
// Here to check if the start point and end point are coincident with any
// obstacle
uint16_t Check_start_end_point(int8_t map[Height][Width],
                               uint16_t start_point[2], uint16_t end_point[2]) {
  uint16_t code = 0;
...
void find_avaiable_point(uint16_t point[2]) {
  ...
  do {
    ...
    for (int i = -expend_index; i <= expend_index; i++) {
      if (map_arr[point[0] - expend_index][point[1] + i] != obstacle_mark &&
          map_arr[point[0] - expend_index][point[1] + i] != unknown_mark) // ->
      {
        point[0] = point[0] - expend_index;
        point[1] = point[1] + i;
        return;
      }
      if (map_arr[point[0] + i][point[1] - expend_index] != obstacle_mark &&
          map_arr[point[0] + i][point[1] - expend_index] !=
              unknown_mark) // go down
      {
        point[0] = point[0] + i;
...
bool map_rec_callback(multiple_rb_ctrl::dynamic_path_srv::Request &req,
  ...
  uint16_t Path[length][2]; // to store a path, can be changed to some variables
                            // array with no length declaration
  clock_t start_time, end_time; // to calculate how long the algorithm takes
  float start[2], end[2];       // received start and goal(assumed)
  uint16_t start_point[2],
      end_point[2]; // transformed start point and end point
  ...
  start[0] = round_coor(
      Odom[req.robot_id - 1]
          .pose.pose.position.y); // row,which is y in grid map coordinate
  ...
  for (int i = 0; i < 4; i++) {
    robot_coordinate[i][0] =
        (uint32_t)(round(Odom[i].pose.pose.position.y / 0.5) + 9);
    robot_coordinate[i][1] =
        (uint32_t)(round(Odom[i].pose.pose.position.x / 0.5) + 9);
  }
  //
  ...
  if (error_code == 0x001) {
    ...
    ROS_ERROR("Error:two points are coincident with obstacles and two points "
              "are coincident,return none");
    search_enable = false;
  }
  if (search_enable == true) {
    ...
    Path_length = A_star_path_finding(map_to_use, start_point, end_point,
                                      Path); // core function
    ...
    default:
      ...
      // test code function:put path in a vector and use a spline function to
      // smooth it
      for (int i = 0; i < Path_length; i++) {
        pathx.push_back(get_map.response.map.info.resolution * Path[i][1] +
                        get_map.response.map.info.origin.position.x);
        pathy.push_back(get_map.response.map.info.resolution * Path[i][0] +
                        get_map.response.map.info.origin.position.y);
      ...
      for (int i = 0; i < Path_length * 2; i++) {
        ...
        /* 				poses[i].header.stamp =
           ros::Time::now(); poses[i].pose.orientation =
...
#endif
      ROS_INFO("Total runtime:%f,There is %d steps in total.",
               (double)(end_time - start_time) / CLOCKS_PER_SEC, Path_length);
      res.Path = path;
    }
...
void odom_1_callback(const nav_msgs::Odometry odom) { Odom[0] = odom; }

void odom_2_callback(const nav_msgs::Odometry odom) { Odom[1] = odom; }

void odom_3_callback(const nav_msgs::Odometry odom) { Odom[2] = odom; }

void odom_4_callback(const nav_msgs::Odometry odom) { Odom[3] = odom; }

int main(int argc, char **argv) {
  ros::init(argc, argv, "path_finding_node");
  ros::NodeHandle nh;
  ros::ServiceServer server =
      nh.advertiseService("path_server", map_rec_callback);
  ros::Subscriber odom_sub1 =
      nh.subscribe<nav_msgs::Odometry>("robot1/odom", 1, odom_1_callback);
  ros::Subscriber odom_sub2 =
      nh.subscribe<nav_msgs::Odometry>("robot2/odom", 1, odom_2_callback);
  ros::Subscriber odom_sub3 =
      nh.subscribe<nav_msgs::Odometry>("robot3/odom", 1, odom_3_callback);
  ros::Subscriber odom_sub4 =
      nh.subscribe<nav_msgs::Odometry>("robot4/odom", 1, odom_4_callback);
  generate_map();
  Odom[0].pose.pose.position.x = 4.5;
  Odom[0].pose.pose.position.y = 2;
  Odom[0].pose.pose.orientation =
      tf::createQuaternionMsgFromRollPitchYaw(0, 0, 3.14159);
  Odom[1].pose.pose.position.x = 4.5;
  Odom[1].pose.pose.position.y = 1;
  Odom[1].pose.pose.orientation =
      tf::createQuaternionMsgFromRollPitchYaw(0, 0, 3.14159);
  Odom[2].pose.pose.position.x = 4.5;
  Odom[2].pose.pose.position.y = -1;
  Odom[2].pose.pose.orientation =
      tf::createQuaternionMsgFromRollPitchYaw(0, 0, 3.14159);
  Odom[3].pose.pose.position.x = 4.5;
  Odom[3].pose.pose.position.y = -2;
  Odom[3].pose.pose.orientation =
      tf::createQuaternionMsgFromRollPitchYaw(0, 0, 3.14159);
