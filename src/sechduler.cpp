...
void generate_new_goal(uint8_t robot_id, multiple_rb_ctrl::instruction_srv::Response &res)
{
  float temp_goal[2];
  switch (robot_id)
  {
  case 1:
    ...
    {
      temp_goal[1] = round_coor((float)rand() / (float)RAND_MAX * 10 - 5);
      if ((temp_goal[1] >= 0.5 && temp_goal[1] <= 3.5) || (temp_goal[1] <= -0.5 && temp_goal[1] >= -3.5))
        break;
    }
